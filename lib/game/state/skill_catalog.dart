...
enum SkillPath { edge, daemon, hex }
...
extension SkillArchetypeExt on SkillArchetype {
  SkillPath get path {
    return switch (this) {
      ...
      SkillArchetype.rupture => SkillPath.edge,
      SkillArchetype.mothership ||
      SkillArchetype.firewall ||
      SkillArchetype.meteor ||
      SkillArchetype.bounty => SkillPath.daemon,
      SkillArchetype.nova ||
      SkillArchetype.frost ||
      SkillArchetype.snake ||
      SkillArchetype.summon => SkillPath.hex,
    };
  }
...
final List<SkillDefinition> skillCatalog = List.unmodifiable([
  SkillDefinition(
    id: 'chain',
    title: 'Neon Katana Chain',
    archetype: SkillArchetype.chain,
  ),
  SkillDefinition(
    id: 'nova',
    title: 'Mana Reactor Nova',
  ...
  SkillDefinition(
    id: 'snake',
    title: 'Fire Snake Ignite',
    archetype: SkillArchetype.snake,
  ),
  SkillDefinition(
    id: 'summon',
    title: 'Fire Wolf Spirit',
...
class SkillEvolution {
  const SkillEvolution({required this.name, required this.description});
  final String name;
...
final List<FusionDefinition> fusionCatalog = [
  ...
  const FusionDefinition(
    ...
    description:
        'Chain hops shred drone armor — chains gain +1 jump per active mothership drone.',
  ...
  const FusionDefinition(
    ...
    name: 'Killcode Edge',
    description:
        'Rupture executes inscribe binary — next blade strike crits and refunds 1 mana.',
    paths: {SkillPath.edge, SkillPath.daemon},
  ),
  ...
  const FusionDefinition(
    ...
    description:
        'Each chain hop inscribes a sigil node; closing the loop triggers a small nova.',
  ...
  const FusionDefinition(
    ...
    name: 'Phantom Frost',
    description:
        'Sentinel blades carry a frost charge — first hit chills, second shatters.',
    paths: {SkillPath.edge, SkillPath.hex},
  ),
  const FusionDefinition(
    id: 'hexcut_mantra',
  ...
  const FusionDefinition(
    ...
    name: 'Sigil Reactor',
    description:
        'Firewall lanes double Nova radius for any pulse cast inside them.',
  ...
  const FusionDefinition(
    ...
    description:
        'Frost kills inscribe gold sigils — stepping on one doubles gold for 3s.',
    paths: {SkillPath.daemon, SkillPath.hex},
  ),
];
...
final Map<SkillArchetype, (SkillEvolution, SkillEvolution)> evolutionCatalog = {
  SkillArchetype.chain: (
    const SkillEvolution(
      name: 'Chainstorm',
      description: 'Stats: +2 jumps, 30% faster snaps.',
    ),
    const SkillEvolution(
      name: 'Tetherblade',
      description: 'Stats: Massive single target damage.',
    ),
  ),
  SkillArchetype.nova: (
    const SkillEvolution(
      name: 'Pulse Reactor',
      description: 'Stats: 40% faster pulse frequency.',
    ),
    const SkillEvolution(
      name: 'Singularity',
      description: 'Special: Novas pull enemies inward.',
    ),
  ),
  SkillArchetype.firewall: (
    const SkillEvolution(
      name: 'Magma Lane',
      description: 'Stats: Deploys 2 walls, double burn.',
    ),
    const SkillEvolution(
      name: 'Dragon Gate',
      description: 'Special: Massive width and knockback.',
    ),
  ),
  SkillArchetype.meteor: (
    const SkillEvolution(
      name: 'Starfall',
      description: 'Stats: 3 extra meteors, faster lock.',
    ),
    const SkillEvolution(
      name: 'Chrome Impact',
      description: 'Stats: 500% DMG, stuns enemies.',
    ),
  ),
  SkillArchetype.barrage: (
    const SkillEvolution(
      name: 'Overdrive',
      description: 'Stats: 40% total attack speed surge.',
    ),
    const SkillEvolution(
      name: 'Blade Waltz',
      description: 'Special: Critical strikes every 3rd hit.',
    ),
  ),
  SkillArchetype.focus: (
    const SkillEvolution(
      name: 'Precision',
      description: 'Stats: Major direct damage surge (+50%).',
    ),
    const SkillEvolution(
      name: 'Overload',
      description: 'Special: Splash damage on every hit.',
    ),
  ),
  SkillArchetype.bounty: (
    const SkillEvolution(
      name: 'Soul Harvest',
      description: 'Stats: +50% gold from all sources.',
    ),
    const SkillEvolution(
      name: 'Jackpot',
      description: 'Special: Chance for massive elite payout.',
    ),
  ),
  SkillArchetype.frost: (
    const SkillEvolution(
      name: 'Glacier',
      description: 'Special: Freeze enemies solid for 1s.',
    ),
    const SkillEvolution(
      name: 'Shatter Bloom',
      description: 'Special: Fold Shatter into base skill.',
    ),
  ),
  SkillArchetype.rupture: (
    const SkillEvolution(
      name: 'Executioner',
      description: 'Stats: +15% execute threshold.',
    ),
    const SkillEvolution(
      name: 'Vulnerability',
      description: 'Special: Wounded take 30% more DMG.',
    ),
  ),
  SkillArchetype.sentinel: (
    const SkillEvolution(
      name: 'Cloud of Steel',
      description: 'Stats: +2 additional blades.',
    ),
    const SkillEvolution(
      name: 'Phantom Edge',
      description: 'Stats: Blades dash 40% faster.',
    ),
  ),
  SkillArchetype.mothership: (
    const SkillEvolution(
      name: 'Armada',
      description: 'Stats: +2 additional seeker drones.',
    ),
    const SkillEvolution(
      name: 'Dreadnought',
      description: 'Stats: One massive 400% DMG drone.',
    ),
  ),
  SkillArchetype.snake: (
    const SkillEvolution(
      name: 'Hydra',
      description: 'Special: Splits into 4 smaller serpents.',
    ),
    const SkillEvolution(
      name: 'World Eater',
      description: 'Stats: One massive double-burn snake.',
    ),
  ),
  SkillArchetype.summon: (
    const SkillEvolution(
      name: 'Spirit Horde',
      description: 'Stats: Summons spawn twice as often.',
    ),
    const SkillEvolution(
      name: 'Avatar',
      description: 'Stats: One massive spirit with all auras.',
    ),
  ),
};
