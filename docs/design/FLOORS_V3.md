# Floors v3 — Past the Architect

*Next-step pass for **Zenith Zero: Idle Descent**.*
*Picks up where Floors v2 ends.*

---

## 0. Why this exists

Floors v2 closed the *texture* gaps inside the F1→F25 arc. The framework now teaches itself: bosses telegraph, new enemies surface counter tips, the Codex tracks discoveries, the modifier catalog is fully wired, and F10/F20 hand out reward-room boons.

What v2 deliberately did not address:
...
- **The reward room is a one-shot.** Two rooms across a 25-floor descent. The pool collapsed from 8 boons to 5 after we dropped the un-buildable ones in v2.
...
v3 is five items that turn the v2 framework into a **replayable** one. Total scope: ~2.5 weeks. The first three close gaps the v2 spec named in its own open-questions section. The last two introduce the smallest amount of new design needed to give the descent a horizon past F25.
...
- The F25 permanent unlock queue from v2 stays sealed (5 entries, claimed once each). Endless does *not* award unlocks — it's pure leaderboard-style depth.
...
The v2 doc explicitly flagged this as open question #2: *"F10/F20 only, or every boss after F10?"* Two rooms gave players a taste; five would let the system breathe.
...
The pool shrank to 5 working boons after v2 dropped `revealModifier`, `halveCantCost`, and `skipNextCant` were trimmed/wired. Add three new boons designed to actually fit existing systems:
...
The lens is the v2 *Prescience Core* boon, but this time with the inter-floor preview UI it needs to mean something. Build it once; reuse it in §3 below.
...
Right now the Codex (with v2's Bestiary and Crucibles tabs) is reachable only through the Nexus Breached death panel. A new player who just discovered the Aegis can't go look at what they discovered until they die.
...
- **No streak rewards.** A streak system implies retention loops we haven't designed yet. v3.5 territory.
...
3. **Reward rooms every 5 floors + 3 new boons** *(3 days)* — answers v2 open Q2; the Modifier Preview Lens unlocks #3's UI.
4. **In-run Codex slate** *(2 days)* — closes the v2 discovery loop the death-screen gating left half-open.
5. **Daily Descent** *(2-3 days)* — last because it's net-new feature surface; the previous four polish what already exists.

Total: ~13-14 working days. Two ~one-week increments, same cadence as v2.

---

## 7. Player-incentive checklist

Same five hooks. v3 reinforces:
...
## 8. What v3 deliberately avoids

- **No new bosses, enemies, or modifiers.** Same v1 catalog. v3 expands the *frame*, not the content.
...
- **No new permanent-unlock entries.** The v2 unlock queue has 5; that's the trophy set. Endless intentionally does not gate behind unlocks.
- **No streak rewards or login bonuses.** v3.5 territory if it ships at all.
...
6. **Run summary social share** — eventually, *PEAK FLOOR* is the kind of stat players screenshot. Does the recap card need to render cleanly when screenshot? Probably yes, but not a blocker for v3.

---

*End of v3 — five items, two-and-a-half weeks, one descent that no longer ends at F25.*
